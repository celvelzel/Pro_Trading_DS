"""
Quant Tool 技术指标模块
整合自 quant_tool/indicators.py，适配 lobster_quant 数据格式
"""
...
def calc_put_call_ratio(calls: list[dict], puts: list[dict]) -> float | None:
...
def calc_max_pain(calls: list[dict], puts: list[dict], current_price: float = 100.0) -> float | None:
    ...
    for strike in all_strikes:
        ...
        for p in puts:
            ...
            intrinsic = max(0, s - current_price)
        ...
        for c in calls:
            ...
            intrinsic = max(0, current_price - s)
...
def find_support_resistance(calls: list[dict], puts: list[dict]) -> tuple[float, float] | None:
