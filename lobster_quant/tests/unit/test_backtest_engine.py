...
from src.data.models import BacktestMetrics, BacktestResult, Strategy, StrategyParams, Trade
...
class TestRunWithStrategy:
    ...
    def test_run_with_strategy_basic(self, mock_get_signal, sample_backtest_data, sample_strategy):
        ...
        mock_scoring.compute_score.return_value = pd.Series(
            [50.0] * len(sample_backtest_data), index=sample_backtest_data.index
        )
    ...
    @patch("src.core.signal_engine.get_signal_engine")
    def test_run_with_strategy_applies_params(
        self, mock_get_signal, sample_backtest_data, sample_strategy
    ):
    ...
    @patch("src.core.signal_engine.get_signal_engine")
    def test_run_with_strategy_restores_on_error(
        self, mock_get_signal, sample_backtest_data, sample_strategy
    ):
    ...
    @patch("src.core.signal_engine.get_signal_engine")
    def test_run_with_strategy_generates_scores(
        self, mock_get_signal, sample_backtest_data, sample_strategy
    ):
        """Test that score series is generated from scoring engine."""
        mock_scoring = MagicMock()
        mock_scores = pd.Series(
            [70.0] * len(sample_backtest_data), index=sample_backtest_data.index
        )
