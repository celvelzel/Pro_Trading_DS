...
class TestOHLCV:
    def test_basic_creation(self):
        candle = OHLCV(
            timestamp=datetime.now(), open=100.0, high=105.0, low=98.0, close=103.0, volume=1000000
        )
    ...
    def test_bearish_candle(self):
        candle = OHLCV(
            timestamp=datetime.now(), open=100.0, high=102.0, low=95.0, close=97.0, volume=500000
        )
...
class TestStockData:
    def test_creation_with_dataframe(self):
        ...
        stock = StockData(symbol="AAPL", daily=df, source="test")
...
class TestSignalResult:
    ...
    def test_neutral_signal(self):
        signal = SignalResult(symbol="AAPL", signal_type="观望", score=25.0, probability_up=40.0)
...
class TestOFFStatus:
    def test_on_status(self):
        status = OFFStatus(timestamp=datetime.now(), is_off=False, reasons=[])
        assert status.is_on is True
        assert status.status_text == "ON"
        assert status.status_emoji == "✅"

    def test_off_status(self):
        status = OFFStatus(timestamp=datetime.now(), is_off=True, reasons=["ATR过高", "大盘风险"])
...
class TestTrade:
    def test_open_trade(self):
        trade = Trade(symbol="AAPL", buy_date=datetime.now(), buy_price=100.0)
...
class TestMarketSnapshot:
    ...
    def test_down_trend(self):
        snapshot = MarketSnapshot(
            code="AAPL", price=150.0, score=30.0, slope_daily=-0.05, slope_weekly=-0.03
        )
