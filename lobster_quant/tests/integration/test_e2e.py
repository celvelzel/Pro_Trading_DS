...
class TestEndToEndWorkflow:
    ...
    def test_backtest_pipeline(self, mock_engine):
        ...
        score_series = pd.Series(
            [signal_gen.calculate_score(df.iloc[: i + 1]) for i in range(len(df))], index=df.index
        )
...
class TestLegacyAdapter:
    ...
    def test_get_signal(self, tmp_path):
        adapter = LegacyAdapter()
        adapter._data_engine = self._create_engine_with_mock(
            tmp_path, trend=0.002, volatility=0.015
        )
