...
class RiskEngine:
    ...
    def assess(
        self, data: pd.DataFrame, benchmark_data: pd.DataFrame | None = None
    ) -> pd.DataFrame:
    ...
    def _assess_benchmark_risk(self, benchmark_data: pd.DataFrame) -> pd.Series | None:
    ...
    def get_stats(self, data: pd.DataFrame, off_results: pd.DataFrame) -> dict[str, Any]:
    ...
    def get_latest_status(
        ...
        if results.empty:
            return OFFStatus(timestamp=datetime.now(), is_off=False, reasons=["Insufficient data"])
