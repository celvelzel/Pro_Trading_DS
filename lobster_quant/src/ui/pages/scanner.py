...
from src.utils.i18n import t, get_signal_label
...
def render_scanner():
    ...
    with col1:
        ...
        if settings.enable_us_stock:
            available_markets.append(t("scanner.us_stocks"))
        if settings.enable_hk_stock:
            available_markets.append(t("scanner.hk_stocks"))
        if settings.enable_a_stock:
            available_markets.append(t("scanner.a_shares"))
        
        selected_market = st.selectbox(
            t("scanner.select_market"),
            options=available_markets,
            index=0
        )
    
    with col2:
        min_score = st.slider(t("scanner.min_score"), 0, 100, settings.backtest_min_score)
    
    # Get stock list
    if selected_market == t("scanner.us_stocks"):
        stock_list = US_STOCK_LIST
    elif selected_market == t("scanner.hk_stocks"):
        stock_list = HK_STOCK_LIST
    else:
        stock_list = A_STOCK_LIST
    
    # Scan button
    if st.button(t("scanner.scan"), type="primary"):
        with st.spinner(t("scanner.scanning", count=len(stock_list))):
            results = scan_stocks(stock_list, engine, min_score)
        
        if results:
            display_results(results)
        else:
            st.info(t("scanner.no_results"))
            st.markdown(f"**{t('scanner.tips_title')}**")
            st.markdown(f"- {t('scanner.tip_lower_score')}")
            st.markdown(f"- {t('scanner.tip_different_market')}")
...
def display_results(results: list) -> None:
    ...
    """
    st.success(t("scanner.found", count=len(results)))
    ...
    df_display = pd.DataFrame([
        {
            'Symbol': r['symbol'],
            'Score': f"{r['score']:.0f}",
            'Signal': get_signal_label(r['signal']),
    ...
    st.subheader(t("scanner.top_picks"))
