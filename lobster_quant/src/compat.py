...
from .core.data_engine import DataEngine, get_data_engine
...
class LegacyAdapter:
    ...
    def fetch_daily(self, symbol: str, years: int = 3) -> pd.DataFrame | None:
    ...
    def fetch_stock_data(self, symbol: str, years: int = 3) -> StockData | None:
        """Fetch complete stock data."""
        return self.data_engine.fetch_stock(symbol, years)

    def fetch_options(self, symbol: str) -> dict | None:
    ...
    def get_signal(self, symbol: str, years: int = 3) -> dict[str, Any] | None:
    ...
    def run_backtest(self, data: pd.DataFrame, symbol: str = "") -> dict[str, Any]:
        ...
        score_series = pd.Series(
            [self.signal_generator.calculate_score(data.iloc[: i + 1]) for i in range(len(data))],
            index=data.index,
        )
