...
def validate_date_range(start: date, end: date, max_years: int = 10) -> tuple[date, date]:
    ...
    if delta > max_years:
        raise ValidationError(f"Date range {delta} years exceeds maximum {max_years} years")
...
def validate_dataframe_columns(
    ...
    if missing:
        raise ValidationError(
            f"{name} missing required columns: {missing}", details={"missing_columns": missing}
        )
...
def validate_timeframe(timeframe: str) -> str:
    ...
    if timeframe not in valid_timeframes:
        raise ValidationError(
            f"Invalid timeframe: '{timeframe}'. " f"Must be one of: {valid_timeframes}"
        )
