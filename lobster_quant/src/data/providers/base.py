...
class DataProvider(ABC):
    ...
    def fetch_stock_data(self, symbol: str, years: int = 3) -> Optional[StockData]:
        ...
        try:
            ...
            return StockData(
                symbol=symbol, daily=daily, weekly=weekly, monthly=monthly, source=self.name
            )
...
class DataProviderFactory:
    ...
    def create(cls, name: str, **kwargs: Any) -> DataProvider:
        ...
        if name not in cls._providers:
            raise ValueError(
                f"Unknown provider: {name}. " f"Available: {list(cls._providers.keys())}"
            )
