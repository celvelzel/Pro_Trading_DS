...
class Indicator(ABC):
    ...
    def validate(self, data: pd.DataFrame) -> bool:
        ...
        if missing:
            raise IndicatorError(self.name, f"Missing required columns: {missing}")

        if len(data) < 20:
            raise IndicatorError(self.name, f"Insufficient data: {len(data)} rows (minimum 20)")
...
class IndicatorRegistry:
    ...
    def get(cls, name: str) -> type[Indicator]:
        ...
        if name not in cls._indicators:
            raise KeyError(
                f"Indicator '{name}' not found. " f"Available: {list(cls._indicators.keys())}"
            )
