...
from quant_tool_data import fetch_daily_data, fetch_option_chain
...
def render_quant_tool_page():
    ...
    with col2:
        fetch_button = st.button("Fetch Data", type="primary", key="quant_tool_fetch")
    
    # Main content area
    if symbol and fetch_button:
        symbol = symbol.strip().upper()
        
        st.success(f"Fetching data for {symbol}...")
        ...
        if options_data is None:
            ...
            st.markdown("### 📊 日内短线建议 - Options Dashboard")
            ...
            if not calls or not puts:
                ...
                st.markdown("#### 📈 期权链 Volume & Open Interest")
        ...
        st.markdown("""
        ### Welcome to Quant Tool
        
        Enter a stock symbol above to analyze:
        - **OFF Filter** - Probability-driven stock screening
        - **Options Analysis** - Max Pain, Support/Resistance
        - **Intraday Recommendations**
        
        Try symbols like: AAPL, MSFT, TSLA, NVDA, MU
        """)
